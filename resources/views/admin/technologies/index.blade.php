                            ...
                            <form action="{{ route('admin.technologies.store') }}" method="post">
                                ...
                                <div class="w-full relative">
                                    <input type="search" id="default-search" name="name"
                                        ...
                                        placeholder="Create a New Tech..." required />
                                    <button type="submit"
                                ...
                                <tr class="bg-white border-b">
                                    <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">
                                        <div class="flex gap-5">
                                            ...
                                            <form action="{{ route('admin.technologies.update', $tech) }}"
                                                method="post">
                                                @csrf
                                                @method('PUT')
                                                <input name="name"
                                                    class="border-0 m-0 py-2.5 ps-4 focus:ring-0 h-full bg-gray-100 rounded-lg"
                                                    type="text" value="{{ $tech->name }}">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <button data-modal-target="modal-{{ $tech->id }}" type="submit"
                                            class="ms-5 text-bold text-red-600 hover:underline">
                                            Delete
                                        </button>

                                        <x-delete-modal :target="$tech" section="technologies" />
                                    </td>
                                </tr>
                            @empty
